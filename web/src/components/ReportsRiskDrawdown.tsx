...
export function ReportsRiskDrawdown({
  ...
  // Stats and series share the ranged window, so "3M" drawdowns measure
  // against the peak within the window, not the all-time peak.
  const visibleTrades = tradesInRange(trades, range);
  const visiblePoints = equityPointsInRange(equityPoints, range);
  const risk = avgRiskPerTrade(visibleTrades);
  const series = drawdownSeries(visiblePoints);
  const maxDd = maxDrawdownPct(visiblePoints);
  const currentDd = currentDrawdownPct(visiblePoints);

...
export function Modal({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange} modal="trap-focus">
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-40 bg-[rgba(5,8,14,0.72)] backdrop-blur-[2px]" />
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
          ...
          >
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-auto p-5">{children}</div>
            {footer && (
              <div className="flex w-full shrink-0 border-t border-border px-5 py-3">{footer}</div>
            )}
          </Dialog.Popup>
        </div>
      </Dialog.Portal>

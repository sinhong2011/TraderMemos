...
function DetailRow({
	...
	return (
		<div className="min-w-0">
			...
			{children ?? (
				<p className={cn("mt-1 truncate text-[13px] tabular-nums", valueClassName)}>
					{value}
				</p>
			)}
		</div>
...
function DetailSection({
	...
	return (
		<section className="flex flex-col gap-2">
			<h3 className="text-[10px] uppercase tracking-[0.12em] text-text-dim">
				{title}
			</h3>
		...
		<Modal open={open} onOpenChange={onOpenChange} title={trade.symbol}>
			<div className="flex flex-col gap-5">
				...
				<DetailSection title="Trade">
					...
					<DetailRow label="Market" value={formatMarketLabel(trade, resolvedOptionRight)} valueClassName="text-text-muted" />
				...
				<DetailSection title="Prices">
					...
					{trade.entry_total != null && trade.entry_total > 0 ? (
						<DetailRow
							label="Entry total"
							value={fmtMoney(trade.entry_total, currency, locale)}
						/>
					) : null}
					{trade.exit_total != null && trade.exit_total > 0 ? (
						<DetailRow
							label="Exit total"
							value={fmtMoney(trade.exit_total, currency, locale)}
						/>
					) : null}
				...
				hasMeaningfulText(trade.tags) ? (
					<DetailSection title="Journal">
						...
						{hasMeaningfulText(trade.stop) ? (
							<DetailRow label="Stop" value={trade.stop} />
						) : null}
						{hasMeaningfulText(trade.tags) ? (

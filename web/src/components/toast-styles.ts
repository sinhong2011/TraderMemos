...
export type SignalToastVariant =
	| "default"
	| "success"
	| "error"
	| "warning"
...
export function resolveToastVariant(
	...
	if (
		/could not|failed|invalid|expired|issues found|upload failed|not remove/.test(
			text,
		)
	) {
		return "error";
	...
	if (
		/saved|created|deleted|removed|uploaded|added|complete|passed|finished/.test(
			text,
		)
	) {
		return "success";

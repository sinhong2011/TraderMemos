	...
	const columns: ColumnDef<JournalTradePreview>[] = [
		{
			...
			header: "Symbol",
			cell: (info) => (
				<span className="font-medium text-text">{info.getValue<string>()}</span>
			),
		},
		{
			id: "market",
			...
			cell: (info) => {
				...
				const needsType =
					trade.instrument_type === "option" && label === "Set type";
				return (
					<span
						className={cn(
		...
		},
		{
			...
			header: "Side",
			cell: (info) => (
				<span className="text-text-muted">{info.getValue<string>()}</span>
			),
		},
		{
			accessorKey: "qty",
		...
		},
		{
			...
			meta: { align: "right" },
			cell: (info) => (
				<span className="tabular-nums">{info.getValue<number>().toFixed(2)}</span>
			),
		},
		{
...
export function csvSampleColumns(
	headers: string[],

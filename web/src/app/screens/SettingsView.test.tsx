...
describe("SettingsView", () => {
	...
	it("marks the only account as primary and hides delete", async () => {
		...
		expect(
			screen.queryByRole("button", { name: /delete main/i }),
	...
	it("allows deleting a secondary account when two exist", () => {
		...
		expect(
			screen.getByRole("button", { name: /delete paper/i }),
		).toBeInTheDocument();
	});
	...
	it("renders the language select on the general tab", async () => {
		...
		renderSettings({ ...baseProps });
		expect(
			await screen.findByRole("combobox", { name: /language selector/i }),
		).toBeInTheDocument();
	});
	...
	it("updates settings labels when language changes", async () => {
		...
		expect(
			await screen.findByRole("link", { name: /^一般$/i }),
		).toBeInTheDocument();
	...
	it("renders accounts section header", () => {
		renderSettings({ ...baseProps });
		expect(
			screen.getByRole("heading", { name: "Accounts", level: 2 }),
		).toBeInTheDocument();
	});

...
export function SettingsShell({
	nav,
	children,
}: { nav: ReactNode; children: ReactNode }) {
	return (
		<div className="flex min-h-[calc(100vh-52px)] flex-col lg:flex-row">
			<aside className="shrink-0 bg-bg px-3 py-3 lg:w-[220px] lg:py-5">
				{nav}
			</aside>
...
export function SettingsGroup({
	...
	return (
		<div
			className={cn(
				"overflow-hidden rounded-card bg-bg-panel",
				className,
			)}
		>
			{children}
...
export function SettingsSection({
	...
	return (
		<section id={id} className="scroll-mt-4">
			...
			{children}
			{footnote ? (
				<p className="mt-2 px-1 text-[11px] leading-relaxed text-text-dim">
					{footnote}
				</p>
			) : null}
		</section>
...
export function SettingsInsetForm({ children }: { children: ReactNode }) {
	return (
		<div className="mb-4 overflow-hidden rounded-card bg-bg-panel p-4">
			{children}
		</div>
	);
}
...
function SettingsStat({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex flex-col items-end gap-0.5">
			<span className="text-[13px] font-medium tabular-nums tracking-tight text-text">
				{value}
			</span>
...
export function AccountRow({
	...
	const meta = [
		...
		accountType,
		currency,
		cashCount > 0 ? `${cashCount} cash tx` : null,
	]
		.filter(Boolean)
		.join(" · ");
		...
		>
			<div className="min-w-0 flex-1">
				<div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
					...
					{isPrimary ? (
						<span className="text-[11px] text-text-muted">Primary</span>
					) : null}
				</div>
...
export function SavedBadge({ show }: { show: boolean }) {
	if (!show) return null;
	return (
		<span className="inline-flex items-center gap-1 text-[11px] text-profit">
			Saved
		</span>
	);
}
	...
	if (confirm) {
		return (
			<span className="flex max-w-[220px] flex-col items-end gap-1.5">
				{detail ? (
					<span className="text-right text-[10px] leading-snug text-text-dim">
						{detail}
					</span>
				) : null}
	...
	if (confirm) {
		return (
			<div className="flex w-full max-w-[280px] flex-col items-end gap-2 rounded-control border border-loss/25 bg-bg-inset p-2.5">
				<p className="m-0 text-right text-[10px] leading-snug text-text-dim">
					{detail}
				</p>

...
import type { Summary, EquityPoint, Trade, Account } from "../../lib/api/types";
...
vi.mock("../../components/DataTable", () => ({
  ...
  DataTable: ({ data, columns }: { data: Record<string, unknown>[]; columns: { accessorKey?: string; cell: (ctx: { getValue: () => unknown }) => React.ReactNode }[] }) => (
    <table>
      <tbody>
        {data.map((row, ri) => (
          <tr key={ri}>
            {columns.map((col, ci) => (
              <td key={ci}>
                {col.cell({ getValue: () => col.accessorKey ? row[col.accessorKey] : undefined })}
              </td>
            ))}
          </tr>
        ))}

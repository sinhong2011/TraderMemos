...
function TradeHeader({ trade, onBack }: TradeHeaderProps) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            ...
            style={{
              ...
              color: trade.direction === "long" ? "var(--color-pos)" : "var(--color-neg)",
      ...
      {/* Metadata row */}
      <div className="flex flex-wrap gap-5 text-xs" style={{ color: "var(--color-text-muted)" }}>
        <span>
          <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Opened</span>
          <span className="tabular-nums" style={{ color: "var(--color-text)" }}>{fmtDate(trade.opened_at)}</span>
        </span>
        <span>
          <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Closed</span>
          <span className="tabular-nums" style={{ color: "var(--color-text)" }}>{fmtDate(trade.closed_at)}</span>
        </span>
        <span>
          <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Qty</span>
          <span className="tabular-nums" style={{ color: "var(--color-text)" }}>{trade.qty_opened}</span>
        </span>
        <span>
          <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Entry</span>
        ...
        {trade.avg_exit_price != null && (
          <span>
            <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Exit</span>
            <span className="tabular-nums" style={{ color: "var(--color-text)" }}>
        ...
        <span>
          <span className="uppercase mr-1" style={{ opacity: 0.6 }}>Fees</span>
          <span className="tabular-nums" style={{ color: "var(--color-text)" }}>
          ...
          {fills.map((fill) => (
            <tr key={fill.id}>
              <td
                ...
                style={{
                  color: fill.side === "buy" ? "var(--color-pos)" : "var(--color-neg)",
                  borderBottom: "1px solid var(--color-border)",
                }}
              >
...
export function JournalPanel({
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      {/* Notes */}
      <div>
        <label htmlFor="trade-notes" style={labelStyle}>Notes</label>
        <textarea
          id="trade-notes"
          value={form.notes}
      ...
      {/* Setup */}
      <div>
        <label htmlFor="trade-setup" style={labelStyle}>Setup</label>
        <select
          id="trade-setup"
          value={form.setup_id}
      ...
      {/* Initial risk */}
      <div>
        <label htmlFor="trade-risk" style={labelStyle}>Initial Risk</label>
        <input
          id="trade-risk"
          type="number"
          min="0"
          ...
          <div className="flex flex-col gap-1.5">
            {customTags.map((tag) => (
              <label key={tag.id} className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-xs" style={{ color: "var(--color-text)" }}>{tag.name}</span>
              </label>
            ))}
          </div>
        </div>
...
function ScreenshotsPanel({ attachments, uploading, onUpload, onDelete }: ScreenshotsPanelProps) {

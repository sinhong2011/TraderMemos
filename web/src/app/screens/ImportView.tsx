		...
		<Panel title="Step 1 - Upload CSV">
			<div style={{ padding: "20px 16px" }}>
				<div className="flex flex-col gap-4" style={{ maxWidth: 480 }}>
					...
					{error && (
						<p style={{ fontSize: 11, color: "var(--color-neg)" }}>{error}</p>
					)}
					...
					<div>
						<button
							...
							style={{
								...
								cursor:
									loading || !file || accounts.length === 0
										? "not-allowed"
										: "pointer",
							}}
						>
		...
		<div className="flex flex-col gap-4">
			<Panel title="Step 2 - Map Columns">
				<div style={{ padding: "16px" }}>
					...
					{error && (
						<p
							style={{ fontSize: 11, color: "var(--color-neg)", marginTop: 10 }}
						>
							{error}
						</p>
					)}
...
function Row({
	...
	value,
	highlight,
}: { label: string; value: string; highlight?: "pos" | "neg" }) {
	const color =
		highlight === "pos"
			? "var(--color-pos)"
			: highlight === "neg"
	...
	return (
		...
		>
			...
			<span
				className="tabular-nums"
				style={{ color, fontFamily: "var(--font-mono)", fontWeight: 600 }}
			>
				{value}
			</span>
		</div>

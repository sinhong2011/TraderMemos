...
export function NewTradeDrawer() {
  ...
  async function save() {
    ...
    try {
      ...
      if (primaryTradeId && divAmount != null && divAmount > 0) {
        const signed = side === "short" ? -Math.abs(divAmount) : Math.abs(divAmount);
              ...
              {otherAccounts.map((a) => (
                ...
                >
                  <input
                    ...
                    onChange={() => toggleId(copyAccountIds, a.id, setCopyAccountIds)}
                  />
                  {a.name}
                </label>
              ))}
  ...
  return (
    ...
    >
      <ModalBanner>
        Log any trade you've entered — still open, partially exited, or fully closed. Add buy/sell
        executions, journal notes, and run a compliance check against your risk plan.
      </ModalBanner>
      ...
      {tab === "general" && (
        <>
          ...
          {initialRisk != null && (
            <p className="text-[10px] text-text-muted">Planned risk: ${initialRisk.toFixed(2)}</p>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              onClick={() => setRows((rs) => [...rs, emptyRow(side === "long" ? "buy" : "sell")])}
      ...
      {tab === "journal" && (
        <div className="flex flex-col gap-4">
                ...
                {regularTags.map((t) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => toggleId(selectedTagIds, t.id, setSelectedTagIds)}
                ...
                {mistakeTags.map((t) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => toggleId(selectedMistakeIds, t.id, setSelectedMistakeIds)}
                      className="cursor-pointer border-none bg-transparent p-0"
                    >
          ...
          <div>
            <span className={labelClass}>Screenshots ({pendingFiles.length}/5)</span>
            <input
              type="file"
              accept="image/*"
              multiple
              onChange={(e) => {
                const files = [...(e.target.files ?? [])].slice(0, 5 - pendingFiles.length);
      ...
      {tab === "dividends" && (
        <div className="flex flex-col gap-4">
          <p className="text-xs leading-relaxed text-text-muted">
            Track dividend payouts on this position. Amount rolls into trade total P&amp;L (shorts
            are recorded as a debit). Win/Loss and R stay price-based.
          </p>
          <div className="grid grid-cols-2 gap-3">

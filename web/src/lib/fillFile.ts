/**
 * Local CSV / JSON → TradeExtract for the New Trade prefill (no server round
 * trip — the Import wizard covers whole-account files; this fills one form).
 * Mirrors mobile/src/lib/fill-file.ts.
 *
 * Accepted shapes:
...
function parseSide(raw: unknown): "buy" | "sell" | "" {
  const s = String(raw ?? "")
    ...
    .toLowerCase();
  if (!s) return "";
  const packed = s.replace(/[\s_]/g, "");
  if (/^(sell|sld|sale|s|short|sellshort|selltoopen|selltoclose)$/.test(packed)) return "sell";
  if (/^(buy|bot|b|long|buytoopen|buytoclose|cover)$/.test(packed)) return "buy";
...
function rowFromRecord(record: Record<string, unknown>): ExtractedFill | null {
  ...
  const right = String(mapped.option_right ?? "")
    .trim()
    .toLowerCase();
  const row: ExtractedFill = {
    symbol:
      String(mapped.symbol ?? "")
        .trim()
...
function recordsFromCsv(text: string): Record<string, unknown>[] {
  ...
  if (lines.length < 2) throw new Error("CSV needs a header row and at least one fill row.");
  const headers = splitCsvLine(lines[0]);
  if (
    !headers.some((h) => fieldForHeader(h) === "quantity") ||
    !headers.some((h) => fieldForHeader(h) === "price")
  ) {
    throw new Error("CSV headers must include quantity and price columns.");
  }
...
function recordsFromJson(text: string): Record<string, unknown>[] {
  ...
  if (!Array.isArray(list)) {
    throw new Error(
      "JSON must be an array of fills, or an object with a fills/rows/executions array.",
    );
  }
  return list.filter(
...
export function parseFillFile(name: string, text: string): TradeExtract {
  ...
  if (rows.length === 0) {
    throw new Error("No usable fills found — rows need a positive quantity and price.");
  }

  const extract: TradeExtract = {
    symbol: "",
  ...
  if (skipped > 0) {
    extract.warnings.push(
      `Skipped ${skipped} row${skipped === 1 ? "" : "s"} without quantity/price.`,
    );
  }
  return extract;
}

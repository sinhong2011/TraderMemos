...
export function presetFromRange(from?: string, to?: string, now = new Date()): DateRangePreset {
...
export function formatRangeLabel(from?: string, to?: string, now = new Date()): string {
  ...
  if (fromDay && toDay) {
    const fmt = (d: string) => {
      ...
      return new Date(y, m - 1, day).toLocaleDateString(intlLocale(), {
        month: "short",
        day: "numeric",
        ...(fromDay.slice(0, 4) !== toDay.slice(0, 4) ? { year: "numeric" } : {}),
      });
    };

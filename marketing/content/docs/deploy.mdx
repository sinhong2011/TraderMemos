...
0. Fork / one-click web on your Vercel or Cloudflare — see [Fork & deploy](/en/docs/fork-deploy)
...
Create long-lived tokens in **Settings → API** for MCP, AI agents, or scripts. They have the
same API power as your user account.
...
Raw spec: `/openapi.yaml` on the same host. Docs are public; protect the host with TLS and
network controls as usual.
...
| Goal | Mode |
|------|------|
| Fork / one-click UI on **your** Vercel or CF | [Fork & deploy](/en/docs/fork-deploy) |

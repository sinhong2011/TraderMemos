-- Journal alerts: rule config per user, delivery channels, and fired events.
-- alert_events doubles as the dedupe ledger — the unique (user, rule, dedupe_key)
-- index is what keeps the stateless job runner from re-sending notifications.

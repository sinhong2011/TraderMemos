/**
 * Annual recap computation, ported verbatim from web/src/lib/wrapped.ts —
 * keep the two in sync. Month labels come from Intl at render time on mobile.
 */
...
export function computeYearWrapped(trades: Trade[], year: number): YearWrapped {
  ...
  const insights = computeDashboardInsights(inYear);
